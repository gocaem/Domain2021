...
Public Class DomDetails
            ...
            If Not Session("Domain_ID") Is Nothing And txt_admin_id.Text = "" Then
                ...
                Catch ex As Exception
                    If Not (TypeOf ex Is ThreadAbortException) Then
                        File.AppendAllText(Server.MapPath("ErrorLog\ErrorLog.txt"), Environment.NewLine & "DNS\DomDetails:" + ex.Message + ex.StackTrace & " " + DateTime.Now)
                    End If
                End Try
            End If
        End If
    ...
    Sub Fill_dropdownlists()
        Try
            ...
            Dim Dns_classCmd As New SqlClient.SqlCommand("select_class", Dns_class)
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If Page.IsValid Then
                ...
                ocmd2.Parameters.AddWithValue("oldb_c", Session("BILLING_CONTACT").ToString())
                ocmd2.Parameters.AddWithValue("oldemail", Session("BILLING_EMAIL").ToString())
                ocmd2.Parameters.AddWithValue("oldmob", Session("billing_mob").ToString())
                ocmd2.Parameters.AddWithValue("Userid", Session("Users_ID").ToString())
                ...
                ocmdADNIN.Parameters.AddWithValue("Userid", Session("Users_ID").ToString())
                ocmdADNIN.Parameters.AddWithValue("oldcompany_user_name", Server.HtmlEncode(Session("COMPANY_USER_NAME").ToString()))
                ocmdADNIN.Parameters.AddWithValue("admin_contact", Server.HtmlEncode(Session("ADMIN_CONTACT").ToString()))
                ...
                ocmdADNIN.Parameters.AddWithValue("mobile", Server.HtmlEncode(Session("ad_mobile").ToString()))
                ocmdADNIN.Parameters.AddWithValue("newmobile", Server.HtmlEncode(Mobile.Text))
                ocmdADNIN.Parameters.AddWithValue("email", Server.HtmlEncode(Session("EMAIL").ToString()))
                ...
                ocmdTech.Parameters.AddWithValue("old_c", Session("TECH_CONTACT").ToString())
                ocmdTech.Parameters.AddWithValue("oldemail", Session("TECH_EMAIL").ToString())
                ocmdTech.Parameters.AddWithValue("oldmob", Session("tech_mob").ToString())
                ocmdTech.Parameters.AddWithValue("IPadd", ReusableCode.GetIPAddress)
                ocmdTech.Parameters.AddWithValue("Userid", Session("Users_ID").ToString())
                ocmdTech.Parameters.AddWithValue("ID", Label27.Text)
                ocmdTech.ExecuteNonQuery()
                'update main data
                If (Status.SelectedValue <> Status.SelectedValue) Then
                    'logstatus
                    ...
                    conn_log.Close()
                End If
                If Not EndDate.Text Is Nothing Then
                    ...
                    ocmdDomain.Parameters.AddWithValue("Domain_name", Server.HtmlEncode(Domain_name.Text).ToLower())
                    ocmdDomain.Parameters.AddWithValue("olddomain_name", Server.HtmlEncode(Session("DOMAIN_NAME").ToString()).ToLower())
                    ocmdDomain.Parameters.AddWithValue("Second_name", Server.HtmlEncode(SecondDomain.SelectedValue))
                    ocmdDomain.Parameters.AddWithValue("oldsecond_domain", Server.HtmlEncode(Session("SECOND_DOMAIN_ID").ToString()))
                    ocmdDomain.Parameters.AddWithValue("ClassID", Server.HtmlEncode(Classification.SelectedValue))
                    ocmdDomain.Parameters.AddWithValue("oldClassID", Server.HtmlEncode(Server.HtmlEncode(Session("ClassID").ToString())))
                    ocmdDomain.Parameters.AddWithValue("address", Server.HtmlEncode(Address.Text))
                    ocmdDomain.Parameters.AddWithValue("oldaddress", Server.HtmlEncode(Server.HtmlEncode(Session("addresses").ToString())))
                    ocmdDomain.Parameters.AddWithValue("Nationalno", Server.HtmlEncode(NationalNo.Text))
                    ocmdDomain.Parameters.AddWithValue("OldNationalno", Server.HtmlEncode(Session("Nationalno").ToString()))
                    ocmdDomain.Parameters.AddWithValue("Status_ID", Status.SelectedValue)
                    ocmdDomain.Parameters.AddWithValue("Status_ID", Session("Status_ID").ToString())
                    ocmdDomain.Parameters.AddWithValue("Description", Server.HtmlEncode(OrgDetailTextBox.Text))
                    ocmdDomain.Parameters.AddWithValue("OldDescription", Server.HtmlEncode(Session("OldDescription").ToString()))
                    ocmdDomain.Parameters.AddWithValue("owner", Server.HtmlEncode(OwnerNameTextBox.Text))
                    ocmdDomain.Parameters.AddWithValue("Oldowner", Server.HtmlEncode(Session("Owner_Name").ToString()))
                    ocmdDomain.Parameters.AddWithValue("ORG_EMAIL", Server.HtmlEncode(DEmail.Text))
                    ocmdDomain.Parameters.AddWithValue("oldemail", Session("ORG_EMAIL").ToString())
                    ocmdDomain.Parameters.AddWithValue("fax", Server.HtmlEncode(DEmail.Text))
                    ocmdDomain.Parameters.AddWithValue("oldfax", Session("fax").ToString())
                    ocmdDomain.Parameters.AddWithValue("MOBILE", Server.HtmlEncode(DMobileTextBox.Text))
                    ocmdDomain.Parameters.AddWithValue("oldmobile", Server.HtmlEncode(Session("dom_mob").ToString()))
                    If (CheckBoxFree.Checked) Then
                        ocmdDomain.Parameters.AddWithValue("FREE", "f")
                        ocmdDomain.Parameters.AddWithValue("oldFREE", Session("FREE").ToString())
                    Else
                        ocmdDomain.Parameters.AddWithValue("FREE", "nf")
                        ocmdDomain.Parameters.AddWithValue("oldFREE", Session("FREE").ToString())
                    End If
                    ocmdDomain.Parameters.AddWithValue("Testdomain", TestDomain.Checked)
                    ocmdDomain.Parameters.AddWithValue("oldTestdomain", Session("oldTestdomain").ToString())
                    ocmdDomain.Parameters.AddWithValue("enddate", EndDate.Text)
                    ocmdDomain.Parameters.AddWithValue("oldenddate", Session("END_DATE").ToString())
                    ocmdDomain.Parameters.AddWithValue("regdate", RadDateInput1.Text)
                    ocmdDomain.Parameters.AddWithValue("oldregdate", Session("REG_DATE1").ToString())
                    ocmdDomain.Parameters.AddWithValue("org_name", EntityTextBox.Text)
                    ocmdDomain.Parameters.AddWithValue("Oldorgname", Session("ORG_NAME").ToString())
                    ...
                    ocmdDomain.Parameters.AddWithValue("UserId", Session("Users_ID").ToString())
