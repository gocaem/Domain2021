...
Public Class ActivateThis
        ...
        If Not Page.IsPostBack Then
            ...
            Catch ex As Exception
                ...
                If Not (TypeOf ex Is ThreadAbortException) Then
                    File.AppendAllText(Server.MapPath("ErrorLog\ErrorLog.txt"), Environment.NewLine & "DNS\ActivateThis:" + ex.Message + ex.StackTrace & " " + DateTime.Now)
                End If
            End Try
        End If
    ...
    Private Sub BTN_UPDATE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTN_UPDATE.Click
        Try
            ...
            Dim isitTest As Boolean
            Dim selectindex As Integer
            Dim dd As DateTime = Now
            Dim DDT As String = dd.ToString("dd/MM/yyyy hh:mm:ss tt")
            For Each gridrow In DataGrid1.Items
                selectindex = CType(gridrow.Cells(9).FindControl("RadioButtonList1"), RadioButtonList).SelectedIndex
                isitTest = CType(gridrow.Cells(9).FindControl("ck"), CheckBox).Checked
                If selectindex = 0 Then
                    Try
                        If UPDATE_DNS_ONLINE(CInt(gridrow.Cells(0).Text)) Then

                            If update_domains(CInt(gridrow.Cells(0).Text), CType(gridrow.Cells(10).FindControl("txt_Comment"), TextBox).Text, isitTest) Then
                                If send_admin_EMail(CInt(gridrow.Cells(0).Text), CType(gridrow.Cells(10).FindControl("txt_Comment"), TextBox).Text) Then
                                    lbl_Result.Visible = True
                                    lbl_Result.Text = "Update Successfully"
                                End If
                            End If
                        End If
                    Catch ex As Exception
                ...
                If selectindex = 1 Then
                    Try
                        If UPDATE_DNS_ONLINE(CInt(gridrow.Cells(0).Text)) Then
                            If update_domains_cancel(CInt(gridrow.Cells(0).Text), CType(gridrow.Cells(10).FindControl("txt_Comment"), TextBox).Text, isitTest) Then
                                If send_admin_EMail_cancel(CInt(gridrow.Cells(0).Text), CType(gridrow.Cells(10).FindControl("txt_Comment"), TextBox).Text) Then
                                    lbl_Result.Visible = True
                                    lbl_Result.Text = "Cancel Successfully"
                                End If
                            End If
                        End If
                    Catch ex As Exception
    ...
    Private Function send_admin_EMail_cancel(ByVal domanes_id As Integer, ByVal message_body_text As String) As Boolean
        ...
        Dim recipant_email, recipant_name, Modee_mail, Subject, DOMAIN_NAME, SECOND_DOMAIN, OWNER_NAME, USER_PASSWORD, message_body, message_body1, message_body2, message_body_4NITC, message_body4 As String
        DOMAIN_NAME = ""
        SECOND_DOMAIN = ""
        message_body1 = ""
        message_body4 = ""
        ...
        Try
            ...
            ReusableCode.sndMail(recipant_email, Modee_mail, Subject, message_body)
    ...
    Private Function send_admin_EMail(ByVal domanes_id As Integer, ByVal message_body_text As String) As Boolean
        ...
        Dim recipant_email, mob, recipant_name, nitc_m, Subject, DOMAIN_NAME, SECOND_DOMAIN, OWNER_NAME, USER_PASSWORD, message_body, message_body1 As String
        Subject = ""
        message_body1 = ""
        DOMAIN_NAME = ""
        recipant_email = ""
        ...
        Try
            ...
            If (LangFlag.Contains("en")) Then
                comm2.Parameters.AddWithValue("id", 24)
            Else
                comm2.Parameters.AddWithValue("id", 25)
            End If
            comm2.CommandType = Data.CommandType.StoredProcedure
                red2 = comm2.ExecuteReader()
                While red2.Read
                Subject = red2.Item("EmailName") & " (" & DOMAIN_NAME & SECOND_DOMAIN & ")"
                message_body1 += red2.Item("part1") & " (" & DOMAIN_NAME & SECOND_DOMAIN & ") " & red2.Item("part2") & red2.Item("footer")
            End While
            red2.Close()
                conn2.Close()
    ...
    Private Sub FILL_DATAGRED_delete()
        Try
            ...
            Else
                ...
                DataGrid2.DataBind()
                BTN_UPDATE_delete.Visible = True
            End If
            red1.Close()
            conn.Close()
        Catch ex As Exception
    ...
    Private Sub BTN_UPDATE_delete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BTN_UPDATE_delete.Click
        lbl_Result.Text = ""
        lbl_error.Text = ""
        Try
            ...
            For Each gridrow In DataGrid2.Items
                cchek = CType(gridrow.Cells(9).FindControl("CheckBox1"), CheckBox).Checked
                If cchek Then
                    Try
                        If update_domains_delete(CInt(gridrow.Cells(0).Text), "Delete") Then
                            lbl_Result.Text = "Delete Successfully"
                        End If
                    Catch ex As Exception
                        lbl_error.Text = ex.ToString
                        conn.Close()
